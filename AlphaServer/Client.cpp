...
#define MAX_PACKTES_PER_CLIENT 10
		...
		switch (state) {
			...
			case AlphaClient::GAME: {
				...
				if (!has_been_initialized) {
					...
					sendVar(socket, (unsigned int)66);
					sendVar(socket, (unsigned int)0);
				...
				while (safeRecv(socket, (char*)&packet_id, 1) == 0 && packet_handled_count++ < MAX_PACKTES_PER_CLIENT) {
					...
					switch (packet_id) {
						...
						case 0x0d: {
							...
							rot.y = recvFloat(socket);
							on_ground = recvChar(socket) == 1;
							std::cout << "Updated player position and rotation: " << pos.x << " " << pos.y << " " << pos.z << " / " << rot.x << " " << rot.y << "\n";
							DoCollision(chunks);
							break;
						}
						default: {
							...
							// Remove client
							std::cout << "Kicked client, Invalid or unsupported packet id: " << std::hex << (int)packet_id << std::dec << "\n";
							kicked = true;
							return;
						}
					}
...
void AlphaClient::DoChunks(ChunkManager& chunks) {
	...
	for (int x = chunk_x_center - 2; x <= chunk_x_center + 2; x++) {
		for (int y = chunk_y_center - 2; y <= chunk_y_center + 2; y++) {
...
void AlphaClient::DoCollision(ChunkManager& chunks) {
	...
	if (pos.y < 0) { pos.y = 128; SendPositionAndRotation(); }
