...
struct AlphaClient {
	...
	// X is yaw, Y is pitch
	Vec2f rot;
	double stance = 0; // Weird thing this is
	bool on_ground = true;
	...
	void GenerateHash() {
		...
		if (data == NULL) {
			std::cout << "Failed to generate unique connection hash!\n"; exit(1);
		}
		unique_hash_str = "-";
		free(data);
	}
	...
	void SendPositionAndRotation() {
		...
		sendVar(socket, pos.x);
		sendVar(socket, stance);
		sendVar(socket, pos.y);
		sendVar(socket, pos.z);
		sendVar(socket, rot.x);
		sendVar(socket, rot.y);
	...
	bool EnsureChunkIsLoaded(Vec2i chunk) {
		...
		for (int i = 0; i < loaded_chunks.size(); i++) {
			if (loaded_chunks[i] == chunk) { chunk_should_be_loaded[i] = true; return true; }
		}
