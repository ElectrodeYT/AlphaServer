...
struct Chunk {
	...
	char* blocks_metadata_data;
	char* blocks_light_data;
	char* blocks_sky_light_data;

	inline void SetBlock(Vec3i block, char id) {
	...
	Chunk(int chunk_x, int chunk_y) : x(chunk_x), y(chunk_y) {
		...
		blocks_metadata_data = (char*)malloc(17472);
		blocks_light_data = (char*)malloc(17472);
		blocks_sky_light_data = (char*)malloc(17472);
